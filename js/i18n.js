(function (global) {
  ...
  const dict = {
    zh: {
      ...
      "tec.parsing": "解析中…",
      "tec.noFiles": "尚未選擇檔案。",
      "tec.wbSummary": "共處理 TXT 檔案數：{count}\n{details}\n\nBefore 有效行數：{lines}\nBefore 總號碼筆數（展開後）：{nums}\nBefore 不重複號碼數：{distinct}",
      ...
      "tec.excelSummary": "共處理 Excel 檔案數：{count}\n{details}\n\nAfter 資料列數（有效）：{rows}\nAfter 總號碼筆數（展開後）：{nums}\nAfter 不重複號碼數：{distinct}",
      "tec.excelWarnHeaders": "請檢查 Excel（{names}）：找不到標題列",
      "tec.compareSummary": "總號碼數：{total}\n完全相同：{same}\n有差異：{diff}",
      "tec.sideTotals": "TXT 檔 Number：{txtNum}，B：{txtB}，S：{txtS}\nExcel 檔 Number：{exlNum}，B：{exlB}，S：{exlS}",
